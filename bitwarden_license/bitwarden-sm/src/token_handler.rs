...
//!
//! The handler lives in `bitwarden-sm` (not `bitwarden-auth`) so that `renew_token`
...
impl MiddlewareExt for SecretsManagerTokenHandler {
    ...
    async fn renew_token(&mut self) -> Result<Option<String>, LoginError> {
        ...
        // file and the User key slot is populated, rewrite the encrypted state file so a later
        // cold start can reuse the token without a fresh login.
