    ...
    mod get_enforced {
        ...
        fn send_controls_data_is_parsed() {
            ...
            let views = [typed_view(
                ...
                r#"{"disableSend":true,"whoCanAccess":1,"allowedSendTypes":[0,1],"deletionHours":168}"#,
            )];
            let decision = SendControlsPolicy.get_enforced(org, &views, &[confirmed_member(org)]);
            assert!(decision.enforced);
            assert_eq!(decision.data.disable_send, Some(true));
            assert_eq!(
                decision.data.who_can_access,
                Some(SendAccessControl::PasswordProtected)
            );
            assert_eq!(
                decision.data.allowed_send_types,
                Some(vec![SendType::Text, SendType::File])
            );
            assert_eq!(decision.data.deletion_hours, Some(168));
        }
        ...
        #[test]
        fn malformed_send_controls_data_falls_back_to_default() {
            use crate::{SendControlsPolicy, SendControlsPolicyData};

            let org = OrganizationId::new_v4();
            // An unknown `whoCanAccess` value fails the whole struct parse; it must fall
            // back to `Default` without panicking.
            let views = [typed_view(
                org,
                PolicyType::SendControls,
                r#"{"disableSend":true,"whoCanAccess":99}"#,
            )];
            let decision = SendControlsPolicy.get_enforced(org, &views, &[confirmed_member(org)]);
            assert!(decision.enforced);
            assert_eq!(decision.data, SendControlsPolicyData::default());
        }
    }
