...
///         Ok(Self {
///             key_store: client.get_part().map_err(|e| e.to_string())?,
///             api_config_provider: client.get_part().map_err(|e| e.to_string())?,
///             repository: client.get_part().map_err(|e| e.to_string())?,
///         })
///     }
/// }
/// ```
...
pub fn from_client(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let field_inits = struct_fields.iter().filter_map(|f| {
        ...
        let field_type = &f.ty;
        Some(quote! {
            #field_name: <_ as ::bitwarden_core::client::FromClientPart<#field_type>>::get_part(client).map_err(|e| e.to_string())?
        })
    });
