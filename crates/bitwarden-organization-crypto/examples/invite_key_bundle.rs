...
use bitwarden_organization_crypto::{Invite, InviteBundle, InviteSecret};
...
fn main() {
    ...
    // 1. Create an `InviteBundle`, which consists of two parts.
    let bundle = InviteBundle::make_for_private_key(org_key, &wrapped_private_key, &mut ctx)
        .expect("generating an invite bundle should work");

    // 2. The first part is an `InviteSecret`: the raw URL-fragment secret carried in the invite
    ...
    //
    // This method is named dangerous because it is critical this object is not shared with the
    // server.
    ...
    // }
    // ```
    let invite: &Invite = bundle.get_envelope();
