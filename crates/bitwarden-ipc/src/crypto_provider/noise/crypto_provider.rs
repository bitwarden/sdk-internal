...
fn noise_entry(name: impl Into<String>, endpoint: &Endpoint) -> PerformanceEventDescriptor {
    PerformanceEventDescriptor::new(trace::GROUP, trace::NOISE_TRACK, name)
        .prop("peer", format!("{endpoint:?}"))
}
...
impl NoiseCryptoProvider {
    ...
    {
        let mut event = noise_entry("Handshake (initiator)", &destination).start();

        let result = Self::run_handshake(communication, sessions, destination).await;

        match result {
            Ok(()) => event.prop("outcome", "established"),
            Err(ref error) => event.prop("outcome", format!("{error:?}")),
        }
    ...
    async fn receive(
        ...
        loop {
            ...
            let Ok(transport_frame) = Frame::from_cbor(&message.payload) else {
                ...
                noise_entry("Malformed frame ←", &source_endpoint).log();
                continue;
            };
            ...
            match transport_frame {
                Frame::HandshakeStart(handshake_start) => {
                    ...
                    let mut event = noise_entry("Handshake (responder)", &source_endpoint)
                    ...
                    event.prop("outcome", "established");
                }
                Frame::TransportFrame(transport_frame) => {
                    ...
                    let mut event = noise_entry("Decrypt ←", &source_endpoint)
                    ...
                    let Some(mut state) = crypto_state else {
                        ...
                        event.prop("outcome", "no session; invalidating peer");
                    ...
                    let Ok(payload) = payload else {
                        ...
                        event.prop("outcome", "decryption failed");
                        continue;
                    };
                ...
                Frame::CryptoInvalidated => {
                    noise_entry("Session invalidated by peer", &source_endpoint).log();
                    info!(
