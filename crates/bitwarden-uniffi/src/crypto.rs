use bitwarden_core::key_management::crypto::{
    DeriveKeyConnectorRequest, DerivePinKeyResponse, DerivePrfKeyResponse, EnrollPinResponse,
    InitOrgCryptoRequest, InitUserCryptoRequest, UpdateKdfResponse, UpdatePasswordResponse,
};
...
impl CryptoClient {
    ...
    pub fn derive_prf_key(&self, prf: B64) -> Result<DerivePrfKeyResponse> {
        Ok(self.0.derive_prf_key(prf).map_err(Error::MobileCrypto)?)
    }
