...
use bitwarden_performance_tracking::{PerformanceEvent, PerformanceEventDescriptor};
...
/// carries a user key.
fn trace_request_result<T>(event: &mut PerformanceEvent, result: &Result<T, RequestError>) {
    match result {
        Ok(_) => event.prop("outcome", "answered"),
        Err(error) => event.prop("outcome", format!("{error:?}")),
    }
}
...
fn biometrics_entry(operation: &str, direction: &str) -> PerformanceEventDescriptor {
...
impl RpcHandler for GetBiometricsStatusHandler {
    ...
    async fn handle(&self, request: Self::Request) -> BiometricsStatus {
        let mut event = biometrics_entry("GetStatus", "←")
        ...
        event.prop("status", format!("{status:?}"));
...
impl RpcHandler for UnlockBiometricsHandler {
    ...
    async fn handle(&self, request: Self::Request) -> UnlockBiometricsResponse {
        let mut event = biometrics_entry("Unlock", "←")
            .prop("user_id", request.user_id)
            .start();
        ...
        match user_key {
            Some(_) => event.prop("outcome", "unlocked"),
            None => event.prop("outcome", "canceled or failed"),
        }
...
impl RpcHandler for AuthenticateBiometricsHandler {
    ...
    async fn handle(&self, _: Self::Request) -> bool {
        let mut event = biometrics_entry("Authenticate", "←").start();

        let succeeded = self.biometrics_unlock.authenticate_biometrics().await;

        match succeeded {
            true => event.prop("outcome", "verified"),
            false => event.prop("outcome", "rejected"),
        }
...
pub async fn ipc_request_get_biometrics_status(
    ...
    trace_request_result(&mut event, &result);
    result
}
...
pub async fn ipc_request_authenticate_biometrics(
    ...
    let mut event = biometrics_entry("Authenticate", "→")
        .prop("destination", format!("{:?}", Endpoint::DesktopRenderer))
        .start();
