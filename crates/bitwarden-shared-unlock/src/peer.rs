...
fn announce_entry(
    ...
    PerformanceEventDescriptor::new(trace::GROUP, trace::ANNOUNCES_TRACK, name)
...
impl<D: SharedUnlockDriver + Send + Sync + 'static> SharedUnlockPeer<D> {
    ...
    pub async fn receive_message(
        ...
        match &result {
            Ok(outcome) => event.prop("outcome", outcome.describe()),
            Err(()) => event.prop("outcome", "failed"),
        }
    ...
    async fn send_message(&self, message: SharedUnlockSync, recipient: Endpoint) {
        ...
        let mut event = announce_entry("Sync →", message.user_id, &recipient)
        ...
        match &result {
            ...
            Err(RequestError::Unreachable) => event.prop("outcome", "unreachable"),
