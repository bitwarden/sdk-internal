...
struct SecureChannelCryptoState {
    peer_encapsulation_key: StagedKey<PeerKey<DhEncapsulator<x25519_dalek::PublicKey>>>,
    ...
    should_send_new_key: bool,
}

struct SecureChannelWrapper {
...
struct MessageAD {
    ...
    sender_encapuslation_fingerprint: StagedKey<[u8; 32]>,
    receiver_encapsulation_fingerprint: StagedKey<[u8; 32]>,
    send_key_fingerprint: StagedKey<[u8; 32]>,
    receive_key_fingerprint: StagedKey<[u8; 32]>,

    // to send new keys
    staged_sender_encapsulation_key: Option<[u8; 32]>,
    staged_send_key: Option<Vec<u8>>,
}
...
impl SecureChannelWrapper {
    fn new(mut channel: Box<dyn Channel>) -> Self {
        ...
        let setup_packet = MessageAD {
            rx_counter: 0,
            tx_counter: 0,
            sender_encapuslation_fingerprint: StagedKey {
                active_key: encapsulation_key_hash,
                staged_key: None,
            },
            ...
            staged_sender_encapsulation_key: Some(*encapsulation_key.into_inner().as_bytes()),
            staged_send_key: None,
        };
...
impl Channel for SecureChannelWrapper {
    ...
    fn receive(&mut self) -> Result<Vec<u8>, ChannelError> {
        ...
        if let Some(staged_key) = self.crypto_state.peer_encapsulation_key.staged_key.as_ref() {
            ...
            // );
            if staged_key.fingerprint == packet_metadata.sender_encapuslation_fingerprint.active_key
            {
