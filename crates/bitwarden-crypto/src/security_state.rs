...
impl SecurityState {
    ...
    fn sign(&self, signing_key: &SigningKey) -> Result<SignedSecurityState, CryptoError> {
        Ok(SignedSecurityState(
...
mod tests {
    ...
    fn test_security_state_signing() {
        ...
        let signed_security_state = security_state.sign(&signing_key).unwrap();
