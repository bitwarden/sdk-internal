...
use crate::{rsa::make_key_pair, AsymmetricCryptoKey, AsymmetricEncString, AsymmetricPublicCryptoKey, CryptoError, EncString, KeyDecryptable, KeyEncryptable, SymmetricCryptoKey};
...
impl RotateableKeyset {
    pub fn new(encapsulating_key: &SymmetricCryptoKey, encapsulated_key: &SymmetricCryptoKey) -> Result<Self, CryptoError> {
        let keypair = make_key_pair(encapsulating_key).unwrap();
        let spki = STANDARD.decode(keypair.public.clone()).map_err(|_| CryptoError::InvalidKey)?;
        ...
        let public_key_encrypted_encapsulated_key = AsymmetricEncString::encrypt_rsa2048_oaep_sha1(&encapsulated_key.to_vec(), &pubkey)?;
        Ok(RotateableKeyset {
    ...
    pub fn decrypt_encapsulated_key(&self, encapsulating_key: &SymmetricCryptoKey) -> Result<SymmetricCryptoKey, CryptoError> {
