...
mod tests {
    ...
    // AES-256-CBC-HMAC test vector, generated by `generate_aes256_cbc_hmac_test_vector`. The
    // sealed key now has a thumbprint-derived key id, so the protected header carries a
    // `CONTAINED_KEY_ID` (the "after key id" property).
    const TEST_UNSEALED_AES256_CBC_HMAC_ENCODED: &[u8] = &[
        253, 35, 70, 69, 115, 43, 71, 155, 75, 138, 166, 166, 4, 101, 108, 129, 149, 184, 155, 155,
        190, 242, 210, 240, 60, 180, 198, 210, 164, 70, 214, 204, 191, 187, 76, 12, 45, 148, 233,
        53, 207, 193, 18, 96, 98, 235, 54, 51, 206, 194, 102, 144, 60, 161, 169, 254, 193, 65, 41,
        165, 56, 121, 97, 84,
    ];
    const TESTVECTOR_AES256_CBC_HMAC_ENVELOPE: &[u8] = &[
        132, 88, 74, 165, 1, 3, 3, 120, 34, 97, 112, 112, 108, 105, 99, 97, 116, 105, 111, 110, 47,
        120, 46, 98, 105, 116, 119, 97, 114, 100, 101, 110, 46, 108, 101, 103, 97, 99, 121, 45,
        107, 101, 121, 58, 0, 1, 21, 92, 80, 245, 134, 88, 80, 92, 189, 211, 185, 72, 57, 98, 35,
        114, 179, 5, 232, 58, 0, 1, 56, 129, 6, 58, 0, 1, 56, 128, 32, 161, 5, 76, 167, 50, 178,
        142, 86, 117, 50, 67, 149, 12, 34, 20, 88, 80, 71, 147, 171, 237, 172, 124, 215, 244, 227,
        39, 132, 65, 157, 253, 40, 6, 54, 90, 38, 252, 64, 241, 225, 244, 193, 58, 29, 49, 250,
        242, 98, 133, 169, 161, 115, 252, 213, 131, 240, 189, 200, 34, 71, 170, 20, 91, 141, 170,
        125, 0, 54, 117, 193, 158, 134, 207, 47, 207, 119, 153, 222, 191, 16, 8, 251, 35, 165, 45,
        10, 32, 187, 74, 191, 111, 70, 14, 88, 27, 179, 167, 129, 131, 64, 162, 1, 41, 51, 88, 32,
        145, 190, 162, 226, 212, 7, 11, 70, 155, 168, 59, 205, 199, 51, 60, 50, 213, 43, 131, 139,
        245, 118, 169, 138, 19, 31, 63, 66, 232, 32, 153, 19, 246,
    ];
    ...
    fn test_testvector_aes256_cbc_hmac() {
        let envelope =
            SecretProtectedKeyEnvelope::try_from(&TESTVECTOR_AES256_CBC_HMAC_ENVELOPE.to_vec())
                .expect("Key envelope should be valid");
        let key = envelope
            .unseal_ref(
                &testvector_secret(),
