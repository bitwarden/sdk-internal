...
mod tests {
    ...
    // Predates `CONTAINED_KEY_ID` support: the sealed key is an `Aes256CbcHmacKey` (the legacy
    // BitwardenLegacyKey format) and the envelope's protected header carries no key id at all.
    // This is the "before key id" vector for `Aes256CbcHmacKey`; see
    // `TESTVECTOR_AES256_CBC_HMAC_ENVELOPE` below for the "after key id" counterpart.
    ...
    // AES-256-CBC-HMAC test vector, generated by `generate_aes256_cbc_hmac_test_vector`. Unlike
    // `TESTVECTOR_LEGACYKEY_ENVELOPE` above, `Aes256CbcHmacKey` now has a thumbprint-derived key
    // id, so the protected header carries a `CONTAINED_KEY_ID` — this is the "after key id"
    // counterpart.
    const TEST_UNSEALED_AES256_CBC_HMAC_ENCODED: &[u8] = &[
        103, 84, 66, 240, 78, 105, 231, 134, 188, 110, 76, 207, 158, 98, 244, 75, 116, 199, 100,
        153, 49, 141, 69, 246, 229, 33, 241, 236, 24, 143, 80, 248, 30, 81, 9, 107, 171, 133, 20,
        191, 109, 234, 251, 102, 132, 196, 7, 60, 59, 250, 20, 15, 66, 15, 204, 123, 126, 100, 20,
        177, 106, 33, 145, 179,
    ];
    const TESTVECTOR_AES256_CBC_HMAC_ENVELOPE: &[u8] = &[
        132, 88, 74, 165, 1, 3, 3, 120, 34, 97, 112, 112, 108, 105, 99, 97, 116, 105, 111, 110, 47,
        120, 46, 98, 105, 116, 119, 97, 114, 100, 101, 110, 46, 108, 101, 103, 97, 99, 121, 45,
        107, 101, 121, 58, 0, 1, 21, 92, 80, 23, 80, 185, 166, 94, 122, 111, 15, 107, 147, 102, 49,
        183, 65, 10, 48, 58, 0, 1, 56, 129, 1, 58, 0, 1, 56, 128, 32, 161, 5, 76, 40, 19, 102, 191,
        1, 132, 247, 59, 0, 195, 181, 104, 88, 80, 118, 50, 45, 44, 111, 101, 42, 43, 29, 19, 246,
        121, 30, 178, 222, 159, 220, 55, 225, 197, 185, 193, 19, 74, 211, 110, 134, 18, 49, 92,
        144, 165, 186, 63, 38, 55, 229, 142, 204, 23, 96, 113, 199, 81, 110, 55, 104, 25, 26, 205,
        128, 210, 7, 6, 234, 229, 217, 255, 233, 111, 73, 184, 195, 43, 66, 198, 157, 177, 212,
        247, 108, 199, 242, 66, 190, 159, 241, 100, 142, 198, 129, 131, 71, 161, 1, 58, 0, 1, 21,
        87, 165, 1, 58, 0, 1, 21, 87, 58, 0, 1, 21, 89, 3, 58, 0, 1, 21, 90, 26, 0, 1, 0, 0, 58, 0,
        1, 21, 91, 4, 58, 0, 1, 21, 88, 80, 122, 61, 61, 107, 39, 111, 69, 114, 54, 134, 136, 69,
        1, 57, 112, 45, 246,
    ];
    ...
    #[test]
    #[ignore = "Manual test to generate an AES-256-CBC-HMAC test vector"]
    fn generate_aes256_cbc_hmac_test_vector() {
    ...
    #[test]
    fn test_testvector_aes256_cbc_hmac() {
        let envelope =
            PasswordProtectedKeyEnvelope::try_from(&TESTVECTOR_AES256_CBC_HMAC_ENVELOPE.to_vec())
                .expect("Key envelope should be valid");
        let key = envelope
            .unseal_ref(
                TESTVECTOR_PASSWORD,
        ...
        assert_eq!(
            key.to_encoded().to_vec(),
            TEST_UNSEALED_AES256_CBC_HMAC_ENCODED
        );

        // The sealed key now has a (thumbprint-derived) key id, so the envelope's protected header
        // must carry it as `CONTAINED_KEY_ID` — the "after key id" property.
        let contained_key_id = envelope.contained_key_id().unwrap();
        assert_eq!(key.key_id(), contained_key_id);
        assert!(contained_key_id.is_some());
    }
