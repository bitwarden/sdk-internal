...
mod tests {
    ...
    use super::*;
    use crate::{Aes256CbcHmacKey, SymmetricKeyAlgorithm, traits::tests::TestIds};
    ...
    /// AES-256-CBC-HMAC-AEAD test vector, generated by `generate_aes256_cbc_hmac_test_vectors`.
    /// `DataEnvelope::seal_ref` never produces envelopes using this algorithm (it always uses
    /// AES-256-GCM), so this proves `unseal_ref` can still decrypt one, recovering the algorithm
    /// from the protected header.
    const TEST_VECTOR_AES256_CBC_HMAC_CEK: &str =
        "25fJlch06LkOz9dFXxgkSlPd7loO/Ve5jbnf8jYyrOIZwDSpUikph7VRo+sR7u/z4dH5W6VooX8LXERzk/hETg==";
    const TEST_VECTOR_AES256_CBC_HMAC_ENVELOPE: &str = "";
    ...
    /// Test helper: seals a struct with an AES-256-CBC-HMAC-AEAD content-encryption key.
    /// `DataEnvelope::seal_ref` always uses AES-256-GCM, so this bypasses it to prove that
    /// `unseal_ref` can decrypt envelopes sealed with a different algorithm (the algorithm is
    /// recovered from the protected header on decryption).
    fn seal_ref_aes256_cbc_hmac<T>(
        data: &T,
        namespace: DataEnvelopeNamespace,
    ) -> Result<(DataEnvelope, Aes256CbcHmacKey), DataEnvelopeError>
    where
        T: Serialize + SealableVersionedData,
    {
        // `SymmetricCryptoKey` implements `Drop` (via `ZeroizeOnDrop`), so it cannot be
        // partially-moved out of by destructuring; bind by reference and clone instead.
        let key = SymmetricCryptoKey::make_aes256_cbc_hmac_key();
        let SymmetricCryptoKey::Aes256CbcHmacKey(ref cek) = key else {
            unreachable!("make_aes256_cbc_hmac_key always returns an Aes256CbcHmacKey");
        };
        let cek = cek.clone();
        ...
        let encrypt0 = encrypt_cose0(
            ...
            protected_header,
            &serialized_and_padded_message,
            CoseKeyView::Aes256CbcHmac(&cek).key_bytes(),
        )
        .map_err(|_| DataEnvelopeError::Encoding)?;
    ...
    /// Test helper: unseal an envelope with an AES-256-CBC-HMAC-AEAD content-encryption key.
    fn unseal_with_aes256_cbc_hmac_cek<T>(
        envelope: &DataEnvelope,
        namespace: DataEnvelopeNamespace,
        cek: &Aes256CbcHmacKey,
    ) -> Result<T, DataEnvelopeError>
    where
        T: serde::de::DeserializeOwned + SealableVersionedData,
    {
        envelope.unseal_ref(namespace, CoseKeyView::Aes256CbcHmac(cek))
    }
    ...
    #[test]
    #[ignore]
    fn generate_aes256_cbc_hmac_test_vectors() {
        let data: TestData = TestDataV1 { field: 456 }.into();
        let (envelope, cek) =
            seal_ref_aes256_cbc_hmac(&data, DataEnvelopeNamespace::ExampleNamespace).unwrap();
        let unsealed_data: TestData = unseal_with_aes256_cbc_hmac_cek(
            &envelope,
            DataEnvelopeNamespace::ExampleNamespace,
            &cek,
        )
        ...
        println!(
            "const TEST_VECTOR_AES256_CBC_HMAC_CEK: &str = \"{}\";",
            B64::from(SymmetricCryptoKey::Aes256CbcHmacKey(cek).to_encoded())
        );
        println!(
            "const TEST_VECTOR_AES256_CBC_HMAC_ENVELOPE: &str = \"{}\";",
            String::from(envelope)
        );
    }
    ...
    #[test]
    fn test_data_envelope_aes256_cbc_hmac_test_vector() {
        let cek =
            SymmetricCryptoKey::try_from(B64::try_from(TEST_VECTOR_AES256_CBC_HMAC_CEK).unwrap())
                .unwrap();
        let SymmetricCryptoKey::Aes256CbcHmacKey(ref cek) = cek else {
            panic!("Invalid CEK type");
        };

        let envelope: DataEnvelope = TEST_VECTOR_AES256_CBC_HMAC_ENVELOPE.parse().unwrap();
        let unsealed_data: TestData = unseal_with_aes256_cbc_hmac_cek(
            &envelope,
            DataEnvelopeNamespace::ExampleNamespace,
            cek,
        ...
        .unwrap();
        assert_eq!(unsealed_data, TestDataV1 { field: 456 }.into());
    }
