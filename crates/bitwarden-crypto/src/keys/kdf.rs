...
fn kdf_entry(kdf: &Kdf) -> PerformanceEventDescriptor {
    match kdf {
        Kdf::PBKDF2 { iterations } => {
            PerformanceEventDescriptor::new(trace::GROUP, trace::PBKDF2_TRACK, "Derive key")
        ...
        Kdf::Argon2id {
            ...
            memory,
            parallelism,
        } => PerformanceEventDescriptor::new(trace::GROUP, trace::ARGON2_TRACK, "Derive key")
            .prop("iterations", iterations)
...
impl KdfDerivedKeyMaterial {
    ...
    pub(super) fn derive_kdf_key(
        ...
        let mut event = kdf_entry(kdf).start();
