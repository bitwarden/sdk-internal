use crate::{
    enc_string::encryption_context::{NoContext, NoContextBuilder},
    error::Result,
    AsymmetricCryptoKey, AsymmetricEncString, CryptoError, EncString, KeyDecryptable,
...
pub struct TrustDeviceResponse {
    ...
    pub protected_device_private_key: EncString<NoContext>,
    /// DevicePublicKey encrypted with [UserKey](super::UserKey)
    pub protected_device_public_key: EncString<NoContext>,
}
...
impl DeviceKey {
    ...
    pub fn trust_device(user_key: &SymmetricCryptoKey) -> Result<TrustDeviceResponse> {
        ...
        let protected_device_private_key = device_private_key
            .to_der()?
            .encrypt_with_key(&device_key.0, &NoContext)?;
    ...
    pub fn decrypt_user_key(
        &self,
        protected_device_private_key: EncString<NoContext>,
        protected_user_key: AsymmetricEncString,
    ) -> Result<SymmetricCryptoKey> {
        let device_private_key: Vec<u8> =
            protected_device_private_key.decrypt_with_key(&self.0, &NoContextBuilder)?;
        let device_private_key = AsymmetricCryptoKey::from_der(&device_private_key)?;

        let dec: Vec<u8> =
            protected_user_key.decrypt_with_key(&device_private_key, &NoContextBuilder)?;
