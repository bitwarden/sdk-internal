...
impl SigningKey {
    ...
    pub fn make(algorithm: SignatureAlgorithm) -> Self {
        match algorithm {
            SignatureAlgorithm::Ed25519 => {
                let _event = signature_entry(algorithm, "Generate key pair").start();
    ...
    pub(super) fn sign_raw(&self, data: &[u8]) -> Vec<u8> {
        let _event = signature_entry(self.algorithm(), "Sign").start();
    ...
    fn from_cose(bytes: &CoseKeyBytes) -> Result<Self, EncodingError> {
        ...
        match (&cose_key.alg, &cose_key.kty) {
            ...
            (
                ...
                let kp = {
                    let _event =
                        signature_entry(SignatureAlgorithm::MlDsa44, "Expand signing key").start();
