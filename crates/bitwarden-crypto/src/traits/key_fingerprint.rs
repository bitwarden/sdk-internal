...
/// canonical representation of a public key of a signing, or public-key-encryption key pair, and
/// derive the fingerprint material from that.
///
/// This canonical representation MUST be stable, and MUST not collide with other representations.
/// For key pairs that have multiple components, such as RSA, a valid implementation MUST explain
/// why the chosen representation is canonical and non-colliding.
///
/// It is recommended to use a reasonable cryptographic hashing function, such as SHA-256 to derive
/// the 256-Bit fingerprint from the canonical representation that can have arbitrary length.
/// Once implemented, for a key algorithm type the fingerprint MUST not change, because other
/// cryptographic objects will rely on it, and plugging different fingerprint algorithms for a given
