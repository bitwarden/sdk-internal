...
pub trait Encryptable<Ids: KeyIds, Key: KeyId, Output> {
    fn encrypt(&self, ctx: &mut KeyStoreContext<Ids>, key: Key, content_format: crate::cose::ContentFormat) -> Result<Output, CryptoError>;
}
    ...
    Encryptable<Ids, Key, Vec<Output>> for Vec<T>
{
    fn encrypt(
        ...
        self.iter().map(|value| value.encrypt(ctx, key, content_format)).collect()
    }
}
...
mod tests {
    ...
    fn test_encryptable_bytes() {
        ...
        let vec_encrypted = vec_data.encrypt(&mut ctx, key, ContentFormat::OctetStream).unwrap();
        let slice_encrypted = slice_data.encrypt(&mut ctx, key, ContentFormat::OctetStream).unwrap();
    ...
    fn test_encryptable_string() {
        ...
        let string_encrypted = string_data.encrypt(&mut ctx, key, ContentFormat::OctetStream).unwrap();
        let str_encrypted = str_data.encrypt(&mut ctx, key, ContentFormat::OctetStream).unwrap();
    ...
    fn test_encryptable_option_none() {
        ...
        let string_encrypted = none_data.encrypt(&mut ctx, key, ContentFormat::OctetStream).unwrap();
        ...
        let string_encrypted_bad = none_data.encrypt(&mut ctx, bad_key, ContentFormat::OctetStream).unwrap();
