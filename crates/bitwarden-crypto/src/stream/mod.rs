/// Large blob encryption and decryption needs to be streamed to keep performance reasonable. This
/// module implements streamed encryption, that can be plugged into other IO streaming interfaces,
/// to create a streamed encryption pipeline. Further, it defines the cryptography behind
/// attachments, with the legacy format for compatibility, and a new format that supports
/// random-access decryption, and has enhanced security properties.

use std::ops::Range;

pub(crate) mod aes256_cbc_hmac_legacy_stream;
pub(crate) mod chunked_aead_stream;
...
pub(crate) enum ChunkEncryptionResult {
    ...
    /// The last chunk is emitted and the stream is completed. No more calls to `update` should be made after this.
...
pub(crate) trait StreamingDecryptor: Sized {
    ...
    /// as it can. The caller signals the end of the stream by calling `update` with `last_block = true`.
    /// The decryptor emits the final chunk of plaintext and performs any final authentication checks
    /// if required. The caller MUST verify the presence of the final decrypted chunk before using
    /// the plaintext.
...
pub(crate) trait RandomAccessDecryptor {
    ...
    /// Decrypt the plaintext bytes covering `range` from the encrypted `wire` byte stream.
    fn decrypt_range(&self, wire: &[u8], range: Range<usize>) -> Result<Vec<u8>, Self::Error>;
}
