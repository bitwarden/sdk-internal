...
fn main() {
    // Alice wants to create a message, for which Bob is sure that Alice signed it. Bob should only access the payload
    // if he verified the signatures validity.
    ...
    let signed_object = alice_signature_key.sign(
        &MessageToBob {
    ...
    mock_server.upload("signed_object", signed_object.to_cose().expect("Failed to serialize signed object"));

    // Bob retrieves the signed object from the server
    let retrieved_signed_object = SignedObject::from_cose(
        &mock_server.download("signed_object").expect("Failed to download signed object")
    ...
    let verified_message: MessageToBob = retrieved_signed_object
        .verify_and_unwrap(&alice_verifying_key, &bitwarden_crypto::SigningNamespace::ExampleNamespace)
        .expect("Failed to verify signed object");
    // Bob can now access the content of the message
    println!("Bob received a message from Alice: {}", verified_message.content);
}
