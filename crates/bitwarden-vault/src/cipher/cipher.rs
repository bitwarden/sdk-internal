...
mod tests {
    ...
    fn test_subtitle_ssh_key() {
        let key: SymmetricCryptoKey = "hvBMMb1t79YssFZkpetYsM3deyVuQv4r88Uj9gvYe0+G8EwxvW3v1iywVmSl61iwzd17JW5C/ivzxSP2C9h7Tw==".to_string().try_into().unwrap();
        let crypto = create_test_crypto_with_user_key(key.clone());
        let mut ctx = crypto.context();
        ...
        let subtitle = ssh_key_cipher
            .get_decrypted_subtitle(&mut ctx, SymmetricKeyRef::User)
            .unwrap();
