...
pub fn passport_to_identity(passport: &PassportCredential) -> (Identity, Vec<Field>) {
    ...
    if let Some(field) = create_text_field_from_country_code(passport.issuing_country.as_ref(), "Issuing Country") {
    ...
    if let Some(field) = create_text_field_from_string(passport.issuing_authority.as_ref(), "Issuing Authority") {
        custom_fields.push(field);
    }
    if let Some(field) = create_text_field_from_string(passport.passport_type.as_ref(), "Passport Type") {
        custom_fields.push(field);
    }
...
pub fn person_name_to_identity(person_name: &PersonNameCredential) -> (Identity, Vec<Field>) {
    ...
    if let Some(field) = create_text_field_from_string(person_name.given_informal.as_ref(), "Informal Given Name") {
        custom_fields.push(field);
    }
    if let Some(field) = create_text_field_from_string(person_name.generation.as_ref(), "Generation") {
        custom_fields.push(field);
    }
...
/// - country: EditableField<"country-code"> → Identity::country
/// - All other fields → CustomFields
pub fn drivers_license_to_identity(drivers_license: &DriversLicenseCredential) -> (Identity, Vec<Field>) {
    // Split full name into first and last name if available
    ...
    if let Some(field) = create_text_field_from_date(drivers_license.birth_date.as_ref(), "Birth Date") {
        custom_fields.push(field);
    }
    if let Some(field) = create_text_field_from_date(drivers_license.issue_date.as_ref(), "Issue Date") {
        custom_fields.push(field);
    }
    if let Some(field) = create_text_field_from_date(drivers_license.expiry_date.as_ref(), "Expiry Date") {
        custom_fields.push(field);
    }
    if let Some(field) = create_text_field_from_string(drivers_license.issuing_authority.as_ref(), "Issuing Authority") {
        custom_fields.push(field);
    }
    if let Some(field) = create_text_field_from_string(drivers_license.license_class.as_ref(), "License Class") {
        custom_fields.push(field);
    }
