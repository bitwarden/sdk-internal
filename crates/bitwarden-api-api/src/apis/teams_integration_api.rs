...
pub trait TeamsIntegrationApi: Send + Sync {
    ...
    /// 
    async fn create<'a>(&self, code: Option<&'a str>, state: Option<&'a str>) -> Result<(), Error<CreateError>>;
...
impl TeamsIntegrationApi for TeamsIntegrationApiClient {
    ...
    async fn create<'a>(&self, code: Option<&'a str>, state: Option<&'a str>) -> Result<(), Error<CreateError>> {
        ...
        let local_var_uri_str = format!("{}/organizations/integrations/teams/create", local_var_configuration.base_path);
    ...
    async fn incoming_post(&self, ) -> Result<(), Error<IncomingPostError>> {
        ...
        let local_var_uri_str = format!("{}/organizations/integrations/teams/incoming", local_var_configuration.base_path);
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<IncomingPostError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
    ...
    async fn redirect<'a>(&self, organization_id: uuid::Uuid) -> Result<(), Error<RedirectError>> {
        ...
        let local_var_uri_str = format!("{}/organizations/{organizationId}/integrations/teams/redirect", local_var_configuration.base_path, organizationId=organization_id);
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<RedirectError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
