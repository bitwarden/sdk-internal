...
pub trait AuthRequestsApi: Send + Sync {
    ...
    /// 
    async fn get<'a>(&self, id: uuid::Uuid) -> Result<models::AuthRequestResponseModel, Error<GetError>>;

    /// GET /auth-requests
    /// 
    async fn get_all(&self, ) -> Result<models::AuthRequestResponseModelListResponseModel, Error<GetAllError>>;

    /// GET /auth-requests/pending
    /// 
    async fn get_pending_auth_requests(&self, ) -> Result<models::PendingAuthRequestResponseModelListResponseModel, Error<GetPendingAuthRequestsError>>;

    /// GET /auth-requests/{id}/response
    /// 
    async fn get_response<'a>(&self, id: uuid::Uuid, code: Option<&'a str>) -> Result<models::AuthRequestResponseModel, Error<GetResponseError>>;

    /// POST /auth-requests
    /// 
    async fn post<'a>(&self, auth_request_create_request_model: Option<models::AuthRequestCreateRequestModel>) -> Result<models::AuthRequestResponseModel, Error<PostError>>;

    /// POST /auth-requests/admin-request
    /// 
    async fn post_admin_request<'a>(&self, auth_request_create_request_model: Option<models::AuthRequestCreateRequestModel>) -> Result<models::AuthRequestResponseModel, Error<PostAdminRequestError>>;

    /// PUT /auth-requests/{id}
    /// 
    async fn put<'a>(&self, id: uuid::Uuid, auth_request_update_request_model: Option<models::AuthRequestUpdateRequestModel>) -> Result<models::AuthRequestResponseModel, Error<PutError>>;
}
...
impl AuthRequestsApi for AuthRequestsApiClient {
    ...
    async fn get<'a>(&self, id: uuid::Uuid) -> Result<models::AuthRequestResponseModel, Error<GetError>> {
        ...
        let local_var_uri_str = format!("{}/auth-requests/{id}", local_var_configuration.base_path, id=id);
    ...
    async fn get_pending_auth_requests(&self, ) -> Result<models::PendingAuthRequestResponseModelListResponseModel, Error<GetPendingAuthRequestsError>> {
        ...
        let local_var_uri_str = format!("{}/auth-requests/pending", local_var_configuration.base_path);
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetPendingAuthRequestsError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
    ...
    async fn get_response<'a>(&self, id: uuid::Uuid, code: Option<&'a str>) -> Result<models::AuthRequestResponseModel, Error<GetResponseError>> {
        ...
        let local_var_uri_str = format!("{}/auth-requests/{id}/response", local_var_configuration.base_path, id=id);
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());

        if let Some(ref param_value) = code {
            local_var_req_builder = local_var_req_builder.query(&[("code", &param_value.to_string())]);
        }
        if let Some(ref local_var_token) = local_var_configuration.oauth_access_token {
            local_var_req_builder = local_var_req_builder.bearer_auth(local_var_token.to_owned());
        };
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetResponseError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
    ...
    async fn post_admin_request<'a>(&self, auth_request_create_request_model: Option<models::AuthRequestCreateRequestModel>) -> Result<models::AuthRequestResponseModel, Error<PostAdminRequestError>> {
        ...
        let local_var_uri_str = format!("{}/auth-requests/admin-request", local_var_configuration.base_path);
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<PostAdminRequestError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
