...
pub trait FoldersApi: Send + Sync {
    ...
    /// GET /folders
    /// 
    async fn get_all(&self, ) -> Result<models::FolderResponseModelListResponseModel, Error<GetAllError>>;

    /// POST /folders
    /// 
    async fn post<'a>(&self, folder_request_model: Option<models::FolderRequestModel>) -> Result<models::FolderResponseModel, Error<PostError>>;

    /// PUT /folders/{id}
    /// 
    async fn put<'a>(&self, id: &'a str, folder_request_model: Option<models::FolderRequestModel>) -> Result<models::FolderResponseModel, Error<PutError>>;
}
...
impl FoldersApi for FoldersApiClient {
    ...
    async fn delete<'a>(&self, id: &'a str) -> Result<(), Error<DeleteError>> {
        ...
        let local_var_uri_str = format!("{}/folders/{id}", local_var_configuration.base_path, id=crate::apis::urlencode(id));
    ...
    async fn delete_all(&self, ) -> Result<(), Error<DeleteAllError>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<DeleteAllError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
