...
use bitwarden_crypto::{
    ...
    MasterKey, NoContext, NoContextBuilder, SymmetricCryptoKey, UserKey,
};
...
pub enum AuthRequestMethod {
    ...
    MasterKey {
        ...
        auth_request_key: EncString<NoContext>,
    },
}
...
pub async fn initialize_user_crypto(
    ...
    let private_key: EncString<NoContext> = req.private_key.parse()?;

    match req.method {
        InitUserCryptoMethod::Password { password, user_key } => {
            let user_key: EncString<NoContext> = user_key.parse()?;
...
pub struct DerivePinKeyResponse {
    ...
    encrypted_pin: EncString<NoContext>,
}
...
pub fn derive_pin_user_key(client: &Client, encrypted_pin: EncString<NoContext>) -> Result<EncString<NoContext>> {
    let enc = client.internal.get_encryption_settings()?;
    let user_key = enc.get_key(&None)?;

    let pin: String = encrypted_pin.decrypt_with_key(user_key, &NoContextBuilder)?;
    let login_method = client
        .internal
        .get_login_method()
        .ok_or(Error::NotAuthenticated)?;
...
pub struct DeriveKeyConnectorRequest {
    ...
    pub user_key_encrypted: EncString<NoContext>,
...
pub struct MakeKeyPairResponse {
    ...
    user_key_encrypted_private_key: EncString<NoContext>,
}
...
mod tests {
    ...
    fn test_enroll_admin_password_reset() {
        ...
        let decrypted: Vec<u8> = encrypted
            .decrypt_with_key(&private_key, &NoContextBuilder)
            .unwrap();
    ...
    fn test_make_key_pair() {
        ...
        let private_key: Vec<u8> = encrypted_private_key
            .decrypt_with_key(&user_key.0, &NoContextBuilder)
            .unwrap();
