...
impl Client {
    ...
    /// Create a new Bitwarden client with both a token handler and a cookie middleware.
    ///
    /// Use this constructor when the deployment requires session-affinity cookies injected into
    /// HTTP requests (e.g., self-hosted behind AWS ALB).
