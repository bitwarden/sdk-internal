//! Pin-based unlock in Bitwarden works using a `PasswordProtectedKeyEnvelope`, which is sealed with the
//! PIN and contains the user-key. When unlocking with PIN, the envelope is unsealed with the PIN and the
//! key is loaded into the key-store.
//! 
//! There are two modes of PIN-based unlock: Before-first-unlock (BFU) and after-first-unlock (AFU). In BFU mode, the
//! PIN envelope is persisted to disk. In AFU mode, the PIN envelope is only stored in memory. The memory copy
//! is always loaded into memory when transitioning from BFU to AFU mode with an unlock.

use bitwarden_crypto::{Decryptable, PrimitiveEncryptable, safe::{PasswordProtectedKeyEnvelope, PasswordProtectedKeyEnvelopeNamespace}};
...
use crate::{Client, client::internal::InternalClient, key_management::SymmetricKeySlotId};

/// Pin unlock can be configured to use one of two modes. Before-first-unlock and after-first-unlock.
/// In AFU mode, the PIN is available only after unlocking once with the master password or another unlock method.
/// In BFU mode, PIN unlock is available right after app start. For this, the PIN-encrypted vault key is stored on
/// disk.
...
pub enum PinLockType {
    ...
    BeforeFirstUnlock,
    /// Pin unlock is available after unlocking with another method at least once during the app session
    AfterFirstUnlock,
}
...
pub enum PinUnlockStatus {
    /// A PIN is configured and the PIN envelope is available for decryption, so PIN-based unlock can be attempted.
    Available,
...
/// Provides PIN-based unlock functionality. This includes enrolling into PIN-based unlock, unlocking using the PIN
/// and handling necessary operations (PIN envelope refreshing when transitioning to after-first-unlock mode).
pub struct PinLockSystem<'a> {
    client: &'a Client,
}

impl PinLockSystem<'_> {
    fn internal_client(&self) -> &InternalClient {
    ...
    async fn get_active_pin_envelope(&self) -> Option<PasswordProtectedKeyEnvelope> {
        let mut pin_protected_key_envelope = self.internal_client()
            .state_bridge
            .get_ephemeral_pin_envelope()
            .await;
        if pin_protected_key_envelope.is_none() {
            pin_protected_key_envelope = self.internal_client()     
                .state_bridge
    ...
    pub(crate) async fn unlock(&self, pin: &str) -> Result<(), UnlockError> {
        let pin_envelope = Self::get_active_pin_envelope(self).await.ok_or(UnlockError::NoPinSet)?;

        // Unseal to key ctx
        let mut ctx = self.internal_client().get_key_store().context_mut();
    ...
    pub(crate) async fn on_unlock(&self) -> Result<(), ()> {
        let encrypted_pin = self.internal_client().state_bridge.get_encrypted_pin().await;
    ...
    pub async fn set_pin(&self, pin: String, lock_type: PinLockType) -> Result<(), ()> {
        ...
        self.internal_client().state_bridge.clear_persistent_pin_envelope().await;
        self.internal_client().state_bridge.clear_ephemeral_pin_envelope().await;
        self.internal_client().state_bridge.clear_encrypted_pin().await;

        let pin_envelope = PasswordProtectedKeyEnvelope::seal(SymmetricKeySlotId::User, pin.as_str(), PasswordProtectedKeyEnvelopeNamespace::PinUnlock, &mut self.internal_client().get_key_store().context_mut())
            .map_err(|_| ())?;
        let encrypted_pin = pin.encrypt(&mut self.internal_client().get_key_store().context_mut(), SymmetricKeySlotId::User)
            .map_err(|_| ())?;

        self.internal_client().state_bridge.set_encrypted_pin(encrypted_pin).await;
        self.internal_client().state_bridge
    ...
    pub async fn get_pin_lock_type(&self) -> Option<PinLockType> {
        if self.internal_client().state_bridge.get_persistent_pin_envelope().await.is_some() {
            return Some(PinLockType::BeforeFirstUnlock);
        }

        if self.internal_client().state_bridge.get_ephemeral_pin_envelope().await.is_some() {
    ...
    pub async fn get_pin(&self) -> Option<String> {
        let encrypted_pin = self.internal_client().state_bridge.get_encrypted_pin().await?;
        encrypted_pin.decrypt(&mut self.internal_client().get_key_store().context_mut(), SymmetricKeySlotId::User).ok()
    }
    ...
    pub async fn validate_pin(&self, pin: String) -> bool {
        ...
        pin_envelope.unseal(pin.as_str(), PasswordProtectedKeyEnvelopeNamespace::PinUnlock, &mut self.internal_client().get_key_store().context_mut()).is_ok()
    }
}