...
use crate::{
    ...
    key_management::{
        non_generic_wrappers::PasswordProtectedKeyEnvelope, AsymmetricKeyId,
        MasterPasswordUnlockData, SecurityState, SignedSecurityState, SigningKeyId, SymmetricKeyId,
    },
...
mod tests {
    ...
    use super::*;
    use crate::{
        client::{internal::UserKeyState, test_accounts::test_bitwarden_com_account},
        Client,
    };
    ...
    async fn test_initialize_user_crypto_master_password_unlock() {
        ...
        let InitUserCryptoMethod::Password { password, user_key } = test_account.user.method else {
        ...
        initialize_user_crypto(
            &client,
            InitUserCryptoRequest {
                user_id: test_account.user.user_id,
                kdf_params: test_account.user.kdf_params.clone(),
                email: test_account.user.email.clone(),
                private_key: test_account.user.private_key,
                signing_key: test_account.user.signing_key,
                security_state: test_account.user.security_state,
                method: InitUserCryptoMethod::MasterPasswordUnlock {
                    password,
                    master_password_unlock: MasterPasswordUnlockData {
                        kdf: test_account.user.kdf_params.clone(),
                        master_key_wrapped_user_key: user_key,
                        salt: test_account.user.email.clone(),
                    },
                },
            },
        )
