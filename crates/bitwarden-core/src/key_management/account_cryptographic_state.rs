...
use crate::{UserId, key_management::{AsymmetricKeyId, KeyIds, SecurityState, SignedSecurityState, SigningKeyId, SymmetricKeyId}};
...
pub enum WrappedUserAccountCryptographicState {
    ...
    /// A V2 user has a private key, a signing key, a signed public key and a signed security state. The SignedPublicKey ensures that others can verify the public key is claimed by an identity they want to share data to.
    /// The signed security state protects against cryptographic downgrades.
    V2 {
        /// The user's encryption private key, wrapped by the user key.
        private_key: EncString,
...
impl WrappedUserAccountCryptographicState {
    ...
    pub fn make(store: &KeyStore<KeyIds>, user_id: UserId) -> Result<Self, AccountCryptographyInitializationError> {
        ...
        Ok(WrappedUserAccountCryptographicState::V2 {
            ...
            signed_public_key,
            signing_key: ctx.wrap_signing_key(
                user_key,
                signing_key,
            )?,
    ...
    pub fn set_to_context(&self, store: &KeyStore<KeyIds>, sdk_security_state: &RwLock<Option<SecurityState>>, user_key: &SymmetricCryptoKey) -> Result<(), AccountCryptographyInitializationError> {
        ...
        match self {
            WrappedUserAccountCryptographicState::V1 { private_key } => {
                ...
                let private_key_id = ctx.unwrap_private_key(tmp_user_key_id, private_key)
                    .map_err(|_| AccountCryptographyInitializationError::WrongUserKey)?;

                ctx.move_asymmetric_key(AsymmetricKeyId::UserPrivateKey, private_key_id)?;
            ...
            WrappedUserAccountCryptographicState::V2 {
                ...
                let private_key_id = ctx.unwrap_private_key(tmp_user_key_id, private_key)
                    .map_err(|_| AccountCryptographyInitializationError::WrongUserKey)?;
                let signing_key_id = ctx.unwrap_signing_key(tmp_user_key_id, signing_key)
                    .map_err(|_| AccountCryptographyInitializationError::WrongUserKey)?;

                let security_state: SecurityState = security_state.to_owned()
...
mod tests {
    ...
    fn test_set_to_context_v1_roundtrip() {
        ...
        let private_key_id = temp_ctx.make_private_key(PublicKeyEncryptionAlgorithm::RsaOaepSha1).unwrap();
        let wrapped_private = temp_ctx.wrap_private_key(tmp_user_key_id, private_key_id).unwrap();
        ...
        wrapped.set_to_context(&store, &sdk_security_state, &user_key).unwrap();

        // Assert that the private key and user symmetric key were set in the store
        assert!(store.context().has_asymmetric_key(AsymmetricKeyId::UserPrivateKey));
    ...
    fn test_set_to_context_v2_roundtrip() {
        ...
        // Make keys
        let private_key_id = temp_ctx.make_private_key(PublicKeyEncryptionAlgorithm::RsaOaepSha1).unwrap();
        let signing_key_id = temp_ctx.make_signing_key(SignatureAlgorithm::Ed25519).unwrap();
        ...
        let wrapped_private = temp_ctx.wrap_private_key(tmp_user_key_id, private_key_id).unwrap();
        let wrapped_signing = temp_ctx.wrap_signing_key(tmp_user_key_id, signing_key_id).unwrap();
        ...
        assert!(store.context().has_asymmetric_key(AsymmetricKeyId::UserPrivateKey));
        assert!(store.context().has_signing_key(SigningKeyId::UserSigningKey));
        assert!(store.context().has_symmetric_key(SymmetricKeyId::User));
