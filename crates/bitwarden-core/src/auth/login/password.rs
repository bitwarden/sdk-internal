...
pub(crate) async fn login_password(
    ...
    if let IdentityTokenResponse::Authenticated(r) = &response {
        ...
        let signing_key = r.user_key_encrypted_signing_key.clone().map(|s| s.parse()).transpose()?;

        client
            .internal
            .initialize_user_crypto_master_key(master_key, user_key, private_key, signing_key)?;
    }
