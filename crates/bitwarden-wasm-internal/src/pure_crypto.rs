...
mod tests {
    ...
    const ENCRYPTED_BYTES: &[u8] = &[2, 209, 195, 115, 49, 205, 253, 128, 162, 169, 246, 175, 217, 144, 73, 108, 191, 27, 113, 69, 55, 94, 142, 62, 129, 204, 173, 130, 37, 42, 97, 209, 25, 192, 64, 126, 112, 139, 248, 2, 89, 112, 178, 83, 25, 77, 130, 187, 127, 85, 179, 211, 159, 186, 111, 44, 109, 211, 18, 120, 104, 144, 4, 76, 3];
    ...
    fn test_symmetric_round_trip() {
        let encrypted = PureCrypto::symmetric_encrypt(DECRYPTED.to_string(), KEY_B64.to_string()).unwrap();
        let decrypted = PureCrypto::symmetric_decrypt(encrypted.clone(), KEY_B64.to_string()).unwrap();
        assert_eq!(decrypted, DECRYPTED);
    }

    #[test]
    fn test_symmetric_decrypt_array_buffer() {
        let result = PureCrypto::symmetric_decrypt_array_buffer(ENCRYPTED_BYTES.to_vec(), KEY_B64.to_string());
        assert!(result.is_ok());
    ...
    fn test_symmetric_encrypt_to_array_buffer() {
        let result = PureCrypto::symmetric_encrypt_to_array_buffer(DECRYPTED_BYTES.to_vec(), KEY_B64.to_string());
        assert!(result.is_ok());
        // Cannot test encrypted string content because IV is unique per encryption
    }
    ...
    fn test_symmetric_buffer_round_trip() {
        let encrypted = PureCrypto::symmetric_encrypt_to_array_buffer(DECRYPTED_BYTES.to_vec(), KEY_B64.to_string()).unwrap();
        let decrypted = PureCrypto::symmetric_decrypt_array_buffer(encrypted.clone(), KEY_B64.to_string()).unwrap();
