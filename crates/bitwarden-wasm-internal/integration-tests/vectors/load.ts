...
/** Which accounts a parameterized test wants to run against. */
export interface VectorFilter {
  securityVersion?: number;
  /** Only accounts declaring this unlock method, e.g. `"masterPasswordUnlock"`. */
  unlockMethod?: string;
  names?: string[];
}

/**
 * The vectors matching `filter`, for `it.each` over a set of accounts.
 *
 * Throws on an empty selection: a filter that matches nothing would otherwise run zero cases and
 * report a green suite.
 */
export function selectUserVectors(
  filter: VectorFilter | ((vector: UserVector) => boolean) = {},
  vectors: UserVector[] = loadUserVectors(),
): UserVector[] {
  const matches =
    typeof filter === "function"
      ? vectors.filter(filter)
      : vectors.filter((v) => matchesFilter(v, filter));

  if (matches.length === 0) {
    throw new Error(
      `no user vector matches ${JSON.stringify(filter)}; have ${vectors.map((v) => v.name).join(", ")}`,
    );
  }
...
function matchesFilter(vector: UserVector, filter: VectorFilter): boolean {
  if (
    filter.securityVersion !== undefined &&
    vector.account.securityVersion !== filter.securityVersion
  ) {
    return false;
  }
  if (filter.names !== undefined && !filter.names.includes(vector.name)) {
    return false;
  }
  if (
    filter.unlockMethod !== undefined &&
    !vector.unlockMethods.some((method) => unlockMethodName(method) === filter.unlockMethod)
  ) {
    return false;
  }
...
/** The unlock method a vector declares first, which is how a test opens it unless it says otherwise. */
export function primaryUnlockMethod(vector: UserVector): InitUserCryptoMethod {
  const [method] = vector.unlockMethods;
  if (method === undefined) {
    throw new Error(`user vector ${vector.name} declares no unlock method`);
  }
  return method;
}
