...
export interface MockRequest {
  ...
  /** Request headers, as sent. The emulator reads `Authorization` off these to identify the caller. */
  headers: Headers;
...
/** The `Authorization: Bearer <token>` value, or `undefined` when absent or not bearer. */
export function bearerToken(request: MockRequest): string | undefined {
...
export function installHttpMock(routes: Routes, options: HttpMockOptions = {}): HttpMock {
  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    // reqwest's wasm client passes a `Request`; tests calling `fetch(url, init)` pass headers in
    // `init`. `new Headers(undefined)` is empty, so there is no third case.
