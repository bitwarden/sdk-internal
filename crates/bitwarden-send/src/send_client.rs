...
use bitwarden_crypto::{ContentFormat, Decryptable, EncString, IdentifyKey, PrimitiveEncryptable};
use thiserror::Error;
...
impl SendClient {
    ...
    pub fn encrypt_buffer(&self, send: Send, buffer: &[u8]) -> Result<Vec<u8>, SendEncryptError> {
        ...
        let encrypted = buffer.encrypt(&mut ctx, key, ContentFormat::OctetStream)?;
